...
![Cost per token vs batch size — weight fetch hyperbola flattening into a compute floor](./visual-1-cost-vs-batch.png)
...
![GPU rack layout showing NVLink within rack vs slow scale-out between racks](./visual-2-rack-layout.png)
...
![Pipeline parallelism bubble diagram showing idle time across 4 racks during training](./visual-3-pipeline-bubble.png)
...
![Bar chart showing three roughly equal compute cost buckets: pretraining, RL, and inference](./visual-4-compute-split.png)
...
![Line graph showing cost per token vs context length with a kink at 200K tokens](./visual-5-context-cost.png)
